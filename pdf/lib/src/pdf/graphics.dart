...
class PdfGraphics {
  ...
  void drawEllipse(double x, double y, double r1, double r2, {bool clockwise = true}) {
    moveTo(x, y - r2);
    if (clockwise) {
  ...
  void drawRRect(double x, double y, double w, double h, double rv, double rh) {
    ...
    lineTo(x + w, y + h - rv);
    curveTo(x + w, y + _m4 * rv + h - rv, x + _m4 * rh + w - rh, y + h, x + w - rh, y + h);
    lineTo(x + rh, y + h);
  ...
  void setFont(
    ...
    assert(() {
      if (_page.settings.verbose) {
        _buf.putString(' ' * math.max(0, _commentIndent - _buf.offset + o));
        _buf.putComment('setFont(${font.ref()}, size: $size, charSpace: $charSpace, wordSpace: $wordSpace, scale: $scale, mode: ${mode.name}, rise: $rise)');
      }
      return true;
    }());
  }
  ...
  void drawString(
    ...
    setFont(font, size, charSpace: charSpace, mode: mode, rise: rise, scale: scale, wordSpace: wordSpace);
  ...
  void setFillColor(PdfColor? color) {
    ...
    if (color is PdfColorCmyk) {
      PdfNumList(<double>[color.cyan, color.magenta, color.yellow, color.black]).output(_page, _buf);
      _buf.putString(' k ');
    } else {
      PdfNumList(<double>[color!.red, color.green, color.blue]).output(_page, _buf);
      _buf.putString(' rg ');
    }
  ...
  void setTransform(Matrix4 t) {
    ...
    final s = t.storage;
    PdfNumList(<double>[s[0], s[1], s[4], s[5], s[12], s[13]]).output(_page, _buf);
    _buf.putString(' cm ');
  ...
  void _endToCenterParameters(double x1, double y1, double x2, double y2, bool large, bool sweep, double rx, double ry) {
    ...
    var dTheta = _vectorAngle((x1d - cxd) / rx, (y1d - cyd) / ry, (-x1d - cxd) / rx, (-y1d - cyd) / ry) % (math.pi * 2.0);
  ...
  void _bezierArcFromCentre(double cx, double cy, double rx, double ry, double startAngle, double extent) {
    int fragmentsCount;
    ...
    var kappa = (4.0 / 3.0 * (1.0 - math.cos(halfFragment)) / math.sin(halfFragment)).abs();
    ...
    for (var i = 0; i < fragmentsCount; i++) {
      ...
      c1 = math.cos(theta);
      s1 = math.sin(theta);
      curveTo(cx + rx * (c0 - kappa * s0), cy - ry * (s0 + kappa * c0), cx + rx * (c1 + kappa * s1), cy - ry * (s1 - kappa * c1), cx + rx * c1, cy - ry * s1);
    }
  }
  ...
  void bezierArc(double x1, double y1, double rx, double ry, double x2, double y2, {bool large = false, bool sweep = false, double phi = 0.0}) {
    if (x1 == x2 && y1 == y2) {
